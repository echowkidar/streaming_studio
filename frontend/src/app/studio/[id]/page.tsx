        ...
        <AnimatePresence>
          {activeTab && (
            <motion.div
              ...
              className={cn(
                "bg-[#0b0b12] overflow-hidden flex flex-col shadow-2xl shrink-0 z-30",
                // Desktop: Docked side-by-side with max-width protection so it never squashes stage
                "lg:relative lg:w-80 lg:max-w-[24vw] lg:border-r lg:border-white/5",
                // Mobile: Slide-over overlay modal next to the icon bar
                "fixed inset-y-0 left-12 sm:left-14 z-50 w-80 max-w-[calc(100vw-3.5rem)] border-r border-white/10"
              )}
            >
              {/* Persistent Header with Panel Title & 1-Click Collapse Button (Desktop & Mobile) */}
